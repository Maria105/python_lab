...
class Model():
    def __init__(self):
        self.filename = 'bookphone.txt'

    def add_members(self, member) -> None:
    ...
    def get_members(self, name: str, fname: str) -> BookPhone:
        member = None
        with open(self.filename, 'r') as f:
            for line in f:
                if line == str(name) + ' ' + str(fname) + '\n':
                    ...
                    member = BookPhone(name, fname, phone_number)
                    break
        return member
        ...
        with open(self.filename, 'r') as f:
            ...
            for line in f:
                ...
                if is_name:
                    ...
                    is_name = True
                    yield BookPhone(full_name[0], full_name[1], line)

    def del_members(self, name: str, fname: str) -> None:
        lines = ''
        with open(self.filename, 'r') as f:
            lines = f.readlines()
...
class View():
    ...
    def input_members(self) -> PhoneBookMember:
        name = input("Enter name: ")
        fname = input("Enter fname: ")
        phone_number = input("Enter phone number: ")
        return BookPhone(name, fname, phone_number)
    ...
    def output_members(self, member) -> None:
        if member:
...
class Controller():
    ...
    def run(self) -> None:
        while (True):
            ...
            if choice == 0: break
            if choice == 1:
                self.model.add_members(self.view.input_members())
            elif choice == 2:
